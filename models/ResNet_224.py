...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, n_channel=3, num_classes=2):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(n_channel, 64, kernel_size=7, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
...
def ResNet18_224(n_channel=3,num_classes=10, pretrained=True):
...
def ResNet50_224(n_channel=3, num_classes=10, pretrained=True):
    print("ResNet50")
...
# def test():
#     net = ResNet50_224(n_channel=3, num_classes=10)
